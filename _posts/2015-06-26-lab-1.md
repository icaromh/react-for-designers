...
**If you're comfortable with JavaScript or any of the frontend frameworks like Backbone/Ember/Angular, this tutorial is NOT for you, and you'll be very frustrated with my writing style.** Otherwise, let's get started!

## Time Estimate: 2+ hours

This tutorial should take about 2 hours (and maybe a bit more) to complete.
...
Let me show you what I mean. Here's the React.js code which displays the same Tweet box. **You don't need to follow along yet - just read the code**.
...
**Answer**: Yes, but that's outside the scope of this tutorial - in short, you need to import something called JSX transformer ([here's how](http://facebook.github.io/react/downloads.html)). This step is not necessary on JSBin though. All you need to do to write JSX on JSBin is to (1) add a React library (the one without addons) from the dropdown and (2) select "JSX (React)" from the dropdown menu on the JS view.
...
## Step 6: Implementing State (10 - 20 minutes)
...
**Start with the JSBin from the previous step.** First, we need to **initialize the magic "state" object** - without this, nothing will work.
...
Now, let's check that the magic state object is correctly being set by writing some debug-only code in `render()`.
...
If you write `disabled="true"` or `disabled="false"` in raw HTML it won't work - in raw HTML, you need to remove the `disabled` attribute to enable the button. But React is **not** raw HTML - it follows the following magic:
...
Start with our previous jQuery implementation (**click "Save" below to start editing**).
...
How about in React? You should try doing this on your own. Start with our previous React implementation (**click "Save" below to start editing**).
...
## Step 10: The "Add Photo" Button, in jQuery (15 - 25 minutes)
...
We add the following check to whether or not the button should be disalbed:
...
Start with our previous React implementation (**click "Save" below to start editing**).
...
However, **in JSX, you can't write `if` or `else` inside `{ ... }` in the markup**. You can use a ternary expression (`a ? b : c`) like we did before, but that would be pretty long in this case.
...
**Question**: Why does `this.remainingCharacters()` have `()` but `this.handleChange` and `this.togglePhoto` don't?
...
On the other hand, you **don't** want to actually call the event handler methods in `render()`. You just want to pass references to them. That's why `this.handleChange` and `this.togglePhoto` don't have `()`.
...
## Step 15: Highlighting Overflown Characters in React (10 - 20 minutes)
...
### Like This Tutorial? Please Retweet :)
